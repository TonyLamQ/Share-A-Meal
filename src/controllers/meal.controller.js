...
let controller = {
  validateMeal: (req,res, next) => {
    ...
    try{
        ...
        assert(typeof maxAmountOfParticipants === 'number', 'maxAmountOfParticipants is not found or must be a int');
        assert(typeof price === 'number', 'price is not found or must be a int');
        next();
    } 
    catch(err) {
        const error = {
